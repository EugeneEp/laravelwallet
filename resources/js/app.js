...
$(document).ready(function(){
  ...
  $('#getSms input[type="button"]').click(function(){
          ...
          action = $this.find('input[name="action"]').val();
                    $.ajax({
                        ...
                        dataType: 'json',
                        type: 'post',
                        data: 'phone='+$this.find('#phone').val()+'&type='+sms_type+'&code='+code+'&_token='+token,
                        beforeSend: function() {
                        ...
                        success: function(json) {
                          ...
                          if(json['error'] == undefined){
                            ...
                            if(action == '/sms/send'){
                              ...
                              $this.find('input[name="action"]').val('/sms/check');
                        ...
                        error: function(xhr, status, error) {
                            ...
                            $this.find('.error').show().text(errText);
                            setTimeout(function(){$this.find('.error').hide();}, 3000);
                        }
                    });
  });
                        ...
                        success: function(json) {
                          ...
                          if(json['error'] == undefined){
                            if(action == 'reg'){
                              ...
                              location.href = '/wallet/transactions';
                            }if(action == 'profile'){
  ...
  $(document).on('click', '.btn-csv', function(){
    let data = $('input[name!=_token]', '.search-form').serialize();
    location.href = $(this).attr('data-href') + "?" + data + "&csv=1";
  });
  ...
  $('.search-form').click(function(){
    window.location.href = window.location.href.split('?')[0] + "?" + $('input[name!=_token]', this).serialize();
    return false;
  });
...
function profilePicLoad(){
  $('.profile_pic').each(function(){
    ...
    if(!$this.find('.profile-outer').length && !$this.find('i').length){
      ...
      let img_url = '/storage/photo/'+file+'.png';
